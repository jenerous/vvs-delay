...
def test():
    ...
    try:
        ...
        api_test_2 = API_test()
        add2 = threading.Thread(target=mycrawler.add_api, args=(api_test_2.name, api_test_2.baseurl), kwargs={'get_params_function': api_test_2.get_params, 'function_to_call':  api_test_2.function_to_call})
        add2.start()
        add2.join()
            ...
            with open(log_file_name, 'r') as log_file:
                for l in log_file:
                    if l.startswith('ERROR'):
                        ...
                        if strptime(strftime('%Y-%m-%d %H:%M:%S', time_before_thread_call), '%Y-%m-%d %H:%M:%S') <= strptime(time_str, '%Y-%m-%d %H:%M:%S'):
